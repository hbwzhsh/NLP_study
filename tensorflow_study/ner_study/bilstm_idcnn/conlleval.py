...
import re
import sys
# 打印日志 filename 指定的日志文件名 level:设置日志级别
...
def calculate_metrics(correct, guessed, total):
    #print("correct:{0}, guessed:{0}, total:{0}".format(correct, guessed, total))
...
def report(counts, out=None):
    ...
    print("by_type:{0}".format(by_type))
...
def report_notprint(counts, out=None):
    ...
    for i, m in sorted(by_type.items()):
        line = []
        line.append('%17s: ' % i)
        line.append('accuracy: %6.2f%%; ' % (100.0*(1.0*(c.token_counter-m.fp-m.fn)/c.token_counter)))
...
def return_report(input_file):
    with open_file(input_file,mode='r') as f:
        counts = evaluate(f)
