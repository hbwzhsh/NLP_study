...
class Model(object):
    def __init__(self, config, is_train=True):
        ...
        with tf.variable_scope("optimizer"):
            ...
            #通过指数衰减函数来生成学习率
            self.lr = tf.train.exponential_decay(self.lr,self.global_step,1000,self.decay_rate,staircase=False)
    ...
    def create_feed_dict(self, is_train, batch):
        ...
        # print("segs:{0}".format(segs))

        max_len = len(chars[0])
        # print("max_len:{0}".format(max_len))
        chars = [char_list[0:max_len] for char_list in chars]
        segs = [seg_list[0:max_len] for seg_list in segs]
    ...
    def run_step(self, sess, is_train, batch):
        ...
        #print("-----self.lr:{0}------".format(sess.run(self.lr)))
        if is_train:
    ...
    def test_evaluate(self, sess, data_manager, id_to_tag):
        """
        :param sess: session  to run the model 
        :param data: list of data
        :param id_to_tag: index to tag name
